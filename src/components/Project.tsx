  ...
  ({ index, projectData }, ref) => {
    ...
    return (
      <div
        ...
        onClick={handleClick}
      >
        <ProjectThumbnail img={projectData.thumbnail || '/default-thumbnail.png'} title={projectData.title} />
        ...
        <div className="flex flex-col gap-7 justify-center max-md:gap-2">
          ...
          <div className="text-sm flex flex-col gap-2">
            ...
            <span>👤 {projectData.role} ({projectData.teamSize})</span>
          </div>
          ...
          <div className="flex flex-wrap gap-2 max-sm:mt-3">
            {projectData.techStack.map((tech) => (
              <span key={tech} className="bg-zinc-800 text-zinc-300 text-xs px-2.5 py-1 rounded">
                {tech}
              </span>
            ))}
          </div>
